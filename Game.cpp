...
void Game::Init()
{
    ...
    Text->Load("Structia Panel.ttf", 30);
...
void Game::Render()
{
    ...
    if (this->State == GAME_MENU)
    {
        ...
        Text->RenderText("Press  'W'  or  'S' to select level", 200.0f, this->Height / 2 + 40.0f, 1.0f);
    }
    if (this->State == GAME_WIN)
    {
        Text->RenderText("You WIN!!!", 320.0f, this->Height / 2 - 20.0f, 1.0f, glm::vec3(0.0f, 1.0f, 0.0f));
...
void Game::DoCollisions()
{
    ...
    // 检测PowerUp碰撞，如果碰撞，则激活它们
    for (PowerUp& powerUp : this->PowerUps)
    {
        if (!powerUp.Destroyed)
        {
            // 如果PowerUp超过屏幕底端，则使他们失活，并销毁
...
GLboolean CheckCollision(GameObject& one, GameObject& two) // AABB - AABB 碰撞检测
{
...
Collision CheckCollision(BallObject& one, GameObject& two) // AABB 碰撞检测
{
